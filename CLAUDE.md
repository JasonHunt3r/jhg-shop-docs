...
- `reference-images/` — **gitignored, local only.** An ongoing depository for photos of physical shop things: tool cases, machine labels, stock, cut results. Not a transition artifact like `Evolution/` — this one keeps growing, which is why it stays out of the public repo rather than accumulating binaries in it.
  - **An image at the top level is unread.** Look at it, and write whatever matters into the doc that owns the subject — a fact that lives only in a photo is a fact nobody can grep.
  - Once its content is in the docs, move it to `transcribed/` and rename it from the camera name to what it is (lowercase, underscores, no periods but the extension). Everything under `transcribed/` is evidence for something already written down, never required reading.
  - Because the folder is untracked, **nothing in the docs may depend on an image being present.** Transcribe first; the photo is the receipt, not the record. The folder's own `README.md` carries an index of what each image was transcribed into.
...
| `transition/jhg_session_handoff_20260816.md` | **Current handoff — start here when resuming.** Repo state, the dead-code sweep, **the disabled last depth pass and why nothing in that block may be deleted**, Panel C's `reverse_commands` doc-rule gap, the questions closed by measurement (do not re-open), the open-items list, and four new traps |
| `transition/jhg_session_handoff_20260815_3.md` | Superseded by the above for repo state and open items. **Still the reference for why the finish-corridor invariant was removed rather than ported** (its §2) — read before re-adding anything like it |
| `transition/jhg_session_handoff_20260815_2.md` | Earlier the same day — still the reference for derived `ARC_MAX_R`, the sweep rule, path fidelity, the input guards and the CAMotics virtual cut. **Its open-items list is superseded**; four of its nine entries are closed |
