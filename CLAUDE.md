...
- `reference-images/` — photos of physical shop things (tool cases, machine labels, stock). **An image at the top level is unread: look at it, and write what matters into the doc that owns the subject.** Once its content is in the docs it moves to `reference-images/transcribed/`, where it is kept as evidence rather than as a source. See that folder's `README.md`.
