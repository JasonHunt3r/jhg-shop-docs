...
**1.4 → 1.5 (2026-08-13):** Added CHECK EXISTING RULES BEFORE DEBUGGING and BUG CLASS RECURRENCE sections, plus a shared vocabulary table -- all recovered from a 52-conversation audit of pre-Claude-Code sessions (`jhg_orphaned_knowledge_1_1.md` in `Transition Support Docs/`). The first of these two rules exists because of a failure in the audit session itself: the fix applied to Panel C's plunge-alignment bug was already a documented standing rule (`ARC-FIT PATH ROTATION`, gcode hygiene doc) -- it was re-derived from scratch instead of being found by checking the docs first. The bug was real and the fix was correct either way, but the doc should have been checked before debugging began, not after.
