...
**1.0 → 1.1 (2026-08-14):** Purged claude.ai-era mechanics (`present_files`, sidebar rendering, `/mnt/user-data/uploads/`) now that files are read and written directly on disk in Claude Code — the visualization hierarchy and file rules are restated for the local workflow. Fixed the Phase 4 smoothness-check mode (comparison, matching the When-to-run list). Cross-references converted to relative links. (Lineage note: this document is the ninth revision of `jhg_svg_toolpath_workflow`, renamed with the version reset at migration.)
...
### Level 1 -- Direct SVG Edit (default)

```
Claude edits the SVG file on disk directly → Jason views it in a browser, Preview, or Illustrator
```
...
```
Python script → cairosvg or equivalent → PNG file on disk
```
...
## SVG Validity -- Requirements

Every JHG SVG written to disk must satisfy all of the following, or viewers render nothing or fall back to a download prompt:
...
- **Use distinct filenames for each version of delivered output files.** Browser and app caching will serve stale files if the filename doesn't change. Increment a version suffix on every reissue -- this applies to SVGs, PNGs, and NC files handed to Jason. (Git-tracked docs and generators version through git.)
- **Files Jason provides are read-only.** Never edit an uploaded or externally-supplied file in place -- copy it to the working directory first. Verify every write succeeded by checking the file on disk, not in-memory data.
...
Run the smoothness check in **comparison mode** (each path against the design geometry at its own target offset) on each separated path after splitting and again after endpoint adjustment -- matching the When-to-run list in Comparison Mode above.

---

## Integration with G-Code Pipeline

The finalized SVG bezier toolpath feeds into the Python generator as the source geometry. Three things are directly relevant to how the generator consumes these curves; the full emission methodology (command selection, arc fitting constraints, helical orbit, finish pass sequence, point cleanup) lives in the G-Code Hygiene doc, and the cross-file conventions (offset stacking, PARAMETERS block) in Shop File Standards.
...
*Full widget specification, generator function, and placement conventions: [jhg_scale_widget_1_0.md](jhg_scale_widget_1_0.md)*
...
Zone color codes, orientation vocabulary (TOP/BOTTOM), and fit shorthand (Z1-TIGHT, Z3-LOOSE, etc.) for the neck pocket fit reference diagram are job-specific and live in [jhg_neck_pocket_details.md](jhg_neck_pocket_details.md).
