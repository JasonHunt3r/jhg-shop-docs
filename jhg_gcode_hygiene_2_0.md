...
ARC_MAX_R     = (ARC_MAX_CHORD**2 / 4 + ARC_FIT_TOL**2) / (2 * ARC_FIT_TOL)
                       # = 281.3mm — derived, see below. Never a literal.
```

*(The first three are canonical per job in each generator's PARAMETERS block. `ARC_MAX_R` is derived from two of them and must stay an expression — written as a literal it drifts, which is exactly what happened between 2026-03-23 and 2026-08-15.)*

### ARC_MAX_R is derived, not chosen — Confirmed 2026-08-15

An arc is worth emitting only if its curvature is **resolvable at the tolerance the fitter itself works to**. The sagitta of an arc of radius `r` across the longest permitted chord is `r - sqrt(r² - (ARC_MAX_CHORD/2)²)`. Set that equal to `ARC_FIT_TOL` and solve for `r`:

```
ARC_MAX_R = (ARC_MAX_CHORD² / 4 + ARC_FIT_TOL²) / (2 × ARC_FIT_TOL) = 281.3 mm
```

Above that radius an arc departs from its own chord by **less than the fitter's own tolerance** — G1 is exactly as accurate and strictly cheaper. Below it, the curvature is real and worth an arc. The formula self-corrects: tighten `ARC_FIT_TOL` to 0.05 and the cap derives to 562mm, which is right — a finer tolerance makes finer curvature resolvable.

**What the cap does NOT do: control accuracy.** Swept 50 → uncapped on both body panels, worst deviation measured against the pre-fit point list stays pinned at `ARC_FIT_TOL` the whole way. Its only effect is command count — Panel A 6591 → 3667 NC lines, Panel C 10295 → 7049, moving from the old values to the derived one.

**What it does do: reject degenerate fits.** Uncapped, Panel A emits arcs of r=4,444mm and Panel C **r=91,404mm** — straight lines expressed as 91-metre circles. That is the cap's real job, and the reason it must not simply be removed.

**Provenance, so this is not re-litigated a third time.** Headstocks have shipped 200 since 2026-03-19 and were never changed. Body panels A and C were both born at **100** on 2026-03-23 — a clamp put in to make a specific 170mm-radius degenerate fit unreachable, in the same session that grafted the ARC-FIT PATH ROTATION defect into Panel C. `Big Ears 6` restored Panel C to 200 on 2026-04-06 once the root cause was reattributed; Panel A never re-based and carried 100 until 2026-08-15. **Neither 100 nor 200 was ever a design decision about body panels.**
...
## PATH FIDELITY — the error `ARC_FIT_TOL` does not bound

**Confirmed 2026-08-15.** `ARC_FIT_TOL` is not a bound on path error. It is a bound on path error **at sample points**. The fitter accepts a span when every sample point in it lies within `ARC_FIT_TOL` of the candidate circle; it never checks what the arc does *between* those points. An arc can pass through every sample within 0.1mm and still bow well outside the intended wall in the gaps.

This is the same mechanism as the March 2026 headstock failure recorded under ARC FITTING CONSTRAINTS. The chord limit added in response narrowed the span but did not close the hole — the excursion still happens inside a 15mm chord.

**Measured, both body panels, at their shipped sample densities:**

| | target points → path *(what the fitter checks)* | path → target curve *(the real error)* |
|---|---|---|
| Panel A | 0.099mm | 0.117mm |
| Panel C @ `ARC_MAX_R` 200 *(shipped until 2026-08-15)* | 0.099mm | **0.345mm** |
| Panel C @ derived 281.3 | 0.100mm | 0.174mm |

The left column is pinned at the tolerance because it *is* the fitter's own check, restated. Only the right column sees the bulge. **Any measurement of toolpath accuracy that compares target points to the emitted path is measuring nothing** — it will report `ARC_FIT_TOL` regardless of what the path actually does.

The magnitude is geometry-dependent, not a simple function of any one parameter: across a 50 → uncapped `ARC_MAX_R` sweep on Panel C it moved 0.17 / 0.35 / 0.17 / 0.40 / 0.38, with no monotonic trend. Treat it as an error term to be measured per file, not predicted.

**How it is enforced:**

1. `jobs/path_fidelity.py` measures it — **one shared implementation**, imported by every generator, deliberately not copied per job. Divergent copies of shared logic are how this project shipped one defect in two lineages for five months.
2. Generators wrap `arc_fit_points` so no call site can skip the measurement, and stamp a `PATH FIDELITY` trailer into the NC carrying `PATH_DEV_MAX`, `PATH_DEV_TO`, `PATH_DEV_FROM`, and the span count.
3. `verify_nc.py` **blocks** when `PATH_DEV_MAX` exceeds `PATH_DEV_LIMIT` (0.2mm = 2 × `ARC_FIT_TOL`; a Precaution-level budget, not a measured machining limit). An **absent** stamp is advisory, not blocking — the archived NCs and both frozen fixtures predate it. **Promote absence to blocking once every generator has reissued.**

The measurement must be taken against a snapshot of the point list made *before* `arc_fit_points` runs: the fitter mutates its input, projecting arc endpoints onto the start radius for GRBL. Measuring against the mutated list compares the fitter's output to itself and always reports ~0.

**Open:** `jhg_shop_file_standards_2_0.md` retired the `SAMPLES_PER_CURVE = 60` rule on 2026-08-14 on the strength of a measurement that appears to have used the point-based metric — which cannot see this error mode. Sampling density is precisely the lever that constrains an arc between its endpoints. That conclusion should be re-run with the path-based metric before it is relied on. **Conjecture — not yet re-measured.** Related and unresolved: Panel A parses at `samples_per_curve=30`, Panel C at `60`, a divergence neither doc records.
