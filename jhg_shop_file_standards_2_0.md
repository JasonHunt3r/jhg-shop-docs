...
| `PENULTIMATE PASSES` | Pre-finish go-and-come at `PENULT_LEAVE_1` then `PENULT_LEAVE` proud of target (stepped — see gcode hygiene, Finish Pass Sequence) |
...
**The generator's PARAMETERS block is the canonical home of every concrete value.** Docs cite parameter names; they do not restate numbers as authoritative. Any number appearing in prose or in an example block is **illustrative unless explicitly labeled canonical** — when a doc and a generator disagree, the generator wins, and the doc gets fixed. Tuned values with history (like `POCKET_EXPAND` — see the neck pocket doc) keep that history in code comments next to the definition, where the person changing the value will actually look.

### Parameter Risk Taxonomy

Not all parameters carry the same cost when wrong. Every generator's PARAMETERS block should group or tag its values by class:

| Class | Wrong value costs | Examples | Caution level |
|---|---|---|---|
| **Dimensional** | Wrong-size part — unrecoverable, scraps the blank | `POCKET_EXPAND`, `WALL_OFFSET`, `EMG_OFFSET`, hole diameters | Maximum — keep value history, never change without a test-cut plan |
| **Surface quality** | Sanding time | `ROUGH_LEAVE`, `PENULT_LEAVE_1`, `PENULT_LEAVE`, feed rates | Safe to tune, cheap to iterate — the finish pass always runs at true target, so these cannot change part dimensions |
| **Machine safety** | Broken bit, gouged spoilboard, crash | `SAFE_Z`, `PARK_Z`, `DEPTH_TOTAL`, the depth schedule | Maximum — physical damage; also check bed-flatness constraints before "fixing" apparent shortfalls |

**Example (illustrative — canonical values live in each job's generator):**
...
WALL_OFFSET      = 0.4     # mm - positive = looser pocket        [dimensional]
EMG_OFFSET       = 1.0     # mm - pickup cavity enlargement (stacks on WALL_OFFSET)  [dimensional]
ROUGH_LEAVE      = 2.0     # mm - proud of target after rough passes      [surface quality]
PENULT_LEAVE_1   = 1.15    # mm - first penultimate step in from rough    [surface quality]
PENULT_LEAVE     = 0.3     # mm - final penultimate leave, handed to finish  [surface quality]
...
Must appear in the PARAMETERS block of every generator that uses arc fitting. (This is the convention requirement; the fitting rules, sweep/direction constraints, and the failure mode they prevent live in the G-Code Hygiene doc, ARC FITTING CONSTRAINTS section.)
...
**Confirmed -- denser sampling EXPOSES offset loops, it does not mask them.** Measured on Panel C's 2.0mm rough offset with the loop-excision fix disabled (pre-fix behaviour): **30 samples produces 0 self-crossings, 60 produces 1.** A faithful representation of a tight concavity has a local radius smaller than the offset distance, so a true normal offset there *must* self-intersect; sparse sampling smooths the concavity until it doesn't. The loop at 60 is correct geometry correctly handled by `excise_self_intersections()` -- not a defect introduced by sampling.

*(This overturns a conjecture recorded here earlier the same day -- that 60 samples masked an offset loop rather than curing it. The measurement runs the other way. Retained as a correction rather than deleted: the reasoning was plausible and wrong, and the doc should show which claims survived contact with a measurement.)*

**Still unexplained: the original failure.** The documented signature was a large-radius arc (r=17.5mm, center inside the body) bulging into the part at a concave dip. Panel C carries an arc of exactly that character (r=17.7mm at 30 samples) -- and it *persists at 500 samples* (r=18.2mm), so it is real geometry rather than an undersampling artifact. The stated mechanism (sparse points under-constraining the fitter on the **finish** path, which is read 1:1 from the sampled bezier with no offsetting) is sound in principle and simply does not bite on these two panels' geometry. A concavity tighter than anything in Panel A or Panel C could still trigger it.

**Current guidance, and it has an ordering dependency -- read both halves:**

1. **Loop excision is a precondition for raising sample density.** 60 samples *without* `excise_self_intersections()` is worse than 30: it surfaces true concave-offset loops with nothing in the pipeline to remove them. 60 *with* excision is safe but buys nothing, because `ARC_FIT_TOL` binds at 0.1mm. So the sequence is excision first, density second -- never the reverse.
2. `samples_per_curve = 30` is adequate for current geometries and is what ships. Raise it only for a specific feature with a measurement showing it helps, and only with excision active. If a generator exposes the value, it belongs in the PARAMETERS block like any other tunable.
...
> **Correction, 2026-08-15.** As written below this claim is overstated, and the measurement behind it could not have caught its own limit. `ARC_FIT_TOL` bounds deviation **where the sample points are**; between them the fitted arc is unconstrained and on shipped Panel C bows to **0.345mm**. Every number in this section was obtained by comparing target points to the emitted path -- the same comparison the fitter runs on itself, which returns `ARC_FIT_TOL` by construction no matter what the path does in the gaps. **Re-run 2026-08-15 with the path-based metric, and the conclusion below survives -- for a reason it never identified.** Sample density barely moves accuracy. What it moved was whether the arc fitter tripped an unguarded failure: the body panels did not bound arc sweep, so at the wrong sample count they emitted an arc that closes on itself, which GRBL cuts as a full circle. Panel A did this at 60 samples, Panel C at 30. So `30 is what ships` was right for Panel A, and the Panel A / Panel C 30-vs-60 split was **load-bearing rather than drift** -- each panel sat on the only safe value for its geometry. The sweep rule is now enforced in both generators and gated, and every sample count from 15 to 240 comes back clean on both panels. Sample density is once again a free choice, to be made on file size rather than safety. See PATH FIDELITY and the sweep section in `jhg_gcode_hygiene_2_0.md`. See PATH FIDELITY in `jhg_gcode_hygiene_2_0.md`. Retained rather than rewritten, per this doc's own practice of showing which claims survived contact with a measurement. Both panels land at 0.14--0.19mm worst-case deviation from a dense ground truth regardless of sample count, against `ARC_FIT_TOL = 0.1mm`. Sampling cannot beat the tolerance the fitter re-approximates to. The lever for more accuracy is therefore tightening `ARC_FIT_TOL` (and accepting more, shorter arcs and a bigger file) -- a cost worth paying deliberately when a job needs it, rather than paying it by proxy through a sample count that buys nothing.
...
Current pipeline avoids this: **PyClipper** handles rough/penultimate offsets (passes that leave stock, where miter behavior at corners doesn't reach finished geometry), and the **finish path is read 1:1 from the Illustrator-corrected bezier curve** (see workflow doc, bezier correction algorithm) rather than computed by any offset function.

**Compliance status (fleet check 2026-08-14): only half true.** Headstocks Neo 3/4 and Panel E ship PyClipper offsets and test clean. Body panels A and C **hand-roll** their offsets (finite-difference normals pushed along, no self-intersection removal) — and both have confirmed self-intersection loops in their rough paths at concave features (Panel A 8_1 at the neck pocket; Panel C `_1_4` at X−105 Y160), because a normal-offset can't avoid crossing itself where the offset distance exceeds the local concave radius. PyClipper removes these by construction. Bringing panels A/C onto Clipper offsets is open work; the repo Panel C even carries an unused `MITER_LIMIT = 20.0` constant from the plan that never landed. This is *why* the pipeline is shaped this way -- if a future change reintroduces Shapely (e.g. `medial_axis` for MazeRunr pocket-throat-width -- see MazeRunr doc), this clipping behavior re-arms for anything Shapely touches.
