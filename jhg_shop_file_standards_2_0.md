...
**Current guidance:** `samples_per_curve = 30` is adequate for current geometries and is what ships. Raise it only for a specific feature with a measurement showing it helps. Note the trade if you do: denser sampling is more faithful, and that faithfulness will surface real offset self-intersections at concave features -- which is a reason to keep `excise_self_intersections()` in the path, not a reason to stay coarse. If a generator does expose the value, it belongs in the PARAMETERS block like any other tunable.
