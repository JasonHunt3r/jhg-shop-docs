...
**The stopped cut was 2026-03-25**, on `jhg_body_pnlC_rot17_2.nc` (17:11), during the *"Panel C workfiles analysis and handoff verification"* session — a fresh audit that checked handoff claims against source rather than trusting them. Two cuts were ruined that day: one from this chord, one from suspected panel bow. Four bugs were diagnosed, and **`ARC-FIT PATH ROTATION` was written into `jhg_gcode_hygiene_1_6.md` the same day**, using this exact event as its ~400mm example. Shop time ran out around 19:15; the session ended in documentation rather than a cut.

**Nothing was cut after 2026-03-25.** The April work (Panel A v8, stepped penultimate, Big Ears) was all generation, no cutting — which is why the chord's disappearance from the Big Ears lineage was a side effect of the rebuild rather than a diagnosis, and why the *older* working lineage carried the same bug untouched until 2026-08-13. One defect, fixed twice, four and a half months apart, in two branches unaware of each other.

**Correction (2026-08-15, same day):** an earlier version of this entry claimed the defect survived a `Proven Cuts/` filing and that the folder records *intent, not verification*. **Both wrong — checked, not assumed, on the second pass.** `Proven Cuts/Panel C proven/jhg_body_pnlC_rot17_3.nc` (17:31) is the *corrected* file: zero long moves at depth, passes `verify_nc.py`. The flawed one is `jhg_body_pnlC_rot17_2.nc` (17:11), twenty minutes earlier. Jason's known-good folder held the right file all along. The fix shipped the same day and was never cut because shop time ran out that evening. `verify_nc.py`'s engagement check now catches this shape, which is the point of having a gate that runs rather than a rule that is remembered.
